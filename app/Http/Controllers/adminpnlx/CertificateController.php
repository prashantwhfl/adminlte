...
class CertificateController extends BaseController {
	...
	public $model		=	'Certificates';
	public $sectionName	=	'Certificates';
	public $headingName	=	'Certificates';
	public $sectionNameSingular	=	'Certificate';
	public $sectionNamePlural	='Certificates';
	...
	*/
	public function index(Request $request){
		...
		if (($request->all())) {
			...
			foreach($searchData as $fieldName => $fieldValue){
				if($fieldValue != ""){
					if($fieldName == "name"){
						$DB->where("certificates.name",'like','%'.$fieldValue.'%');
					}
				}
				$searchVariable	=	array_merge($searchVariable,array($fieldName => $fieldValue));
			}
		}
		$model 					= 	$DB->select("certificates.*")->where('is_deleted',0);
	...
	public function edit($modelId = 0){
		$model				=	Certificates::find($modelId);
		if(empty($model)) {
		...
		//s	dd($validator);
		   ...
		   // dd($obj);
			if($request->hasFile('image')){
				...
				$folderPath			=	CERTIFICATE_PARTNER_IMAGE_URL_IMAGE_ROOT_PATH.$folderName;
